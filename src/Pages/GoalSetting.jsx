...
import { setGoals } from "../utils/slices/goalsSlice"
...
const GoalSetting = () => {
  ...
  const [formdata, setFormdata] = useState({
    weightInput: goals.weight || 0,
    steps: goals.steps || 0,
    sleep: goals.sleep || 0,
    calories: goals.calorie || 0,
    water: goals.water || 0,
  })

  const handleSaveGoalsClick = async (e) => {
    e.preventDefault()
    const {weightInput, steps, sleep, calories, water} = formdata
    if(goals.data) {
      // todo: add update goals api call here
    }else {
      try {
        const res = await api.post('/goals', {weight: weightInput, steps, sleep, calorie: calories, water}, {withCredentials: true})
        console.log(res)
        const {weightRes, stepsRes, calorieRes, sleepRes, waterRes} = res.data?.goal
        dispatch(setGoals({weightRes, stepsRes, sleepRes, calorieRes, waterRes}))
      } catch (err) {
      ...
      <div className="bg-black p-4 border border-gray-700 rounded-lg">
        ...
        <form className="grid gap-4 sm:grid-cols-2 mt-6 p-4 rounded-lg border border-gray-700"> 
          <div>
            <div className="mt-6">
              ...
              <div className="flex flex-col items-start justify-center gap-2 mt-4">
                ...
                <input 
                  className="px-4 py-2 border border-gray-600 block rounded-lg w-full" 
                  value={formdata.weightInput}
                  onChange={e => setFormdata(prev => ({
                    ...prev, weightInput: e.target.value
                  }))}
                  min={30} 
                  max={250} 
                  id="goal-setting-weight" 
                  type="number" 
                  placeholder="70" />
                {weight.current?.weight ? <p className="text-gray-400 text-sm">Your current weight: {weight.current.weight} Kg</p>: null}
              </div>
            </div>
            <div className="mt-6">
              <div className="flex gap-4">
          ...
          <button 
            ...
            className="bg-white px-4 py-2 text-black font-medium rounded-lg w-40 cursor-pointer"
          >Save Goals</button>
        </form>
      </div>
